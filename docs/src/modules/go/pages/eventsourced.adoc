...
[source,go]
----
include::example$tck/cmd/tck_shoppingcart/shoppingcart.go[tag=entity-type]
----
...
[source,go]
----
include::example$cloudstate/eventsourced.go[tag=event-sourced-entity-type]
----
...
While this may introduce some overhead in needing to convert from one type to the other, the reason for doing this is that it will allow the service's public interface to evolve independently from its data storage format, which should be private.
...
[source,protobuf]
----
include::example$protobuf/example/shoppingcart/persistence/domain.proto[]
----
...
[source,go]
----
include::example$tck/cmd/tck_shoppingcart/shoppingcart.go[tag=entity-state]
----
...
[source,go]
----
include::example$tck/cmd/tck_shoppingcart/shoppingcart.go[tag=register]
----
...
[source,go]
----
include::example$tck/cmd/tck_shoppingcart/shoppingcart.go[tag=constructing]
----

== Handling commands

An event sourced entity implements the composed `cloudstate.Entity` interface.
`cloudstate.Entity` embeds the `cloudstate.EventHandler` interface and therefore entities implementing it get commands from Cloudstate through the event handler's `HandleCommand` method.
...
The Cloudstate Go Support library together with the registered `cloudstate.EventSourcedEntity` is then able to dispatch commands it gets from the Cloudstate proxy to the event sourced entity.

[source,go]
----
include::example$tck/cmd/tck_shoppingcart/shoppingcart.go[tag=handle-command]
----
...
[source,go]
----
include::example$tck/cmd/tck_shoppingcart/shoppingcart.go[tag=get-cart]
----
...
A command handler may emit an event by using the embedded `cloudstate.EventEmitter` and invoking the `Emit` method on it.
...
[source,go]
----
include::example$tck/cmd/tck_shoppingcart/shoppingcart.go[tag=add-item]
----

This command handler also validates the command, ensuring the quantity of items added is greater than zero.
Returning an `error` fails the command and the support library takes care of signaling that back to the requesting proxy as a `Failure` reply.
...
[source,go]
----
include::example$cloudstate/event.go[tag=event-handler]
----
...
[source,go]
----
include::example$tck/cmd/tck_shoppingcart/shoppingcart.go[tag=handle-event]
----
...
[source,go]
----
include::example$tck/cmd/tck_shoppingcart/shoppingcart.go[tag=item-added]
----
...
To produce a snapshot, the `cloudstate.Snapshotter` interface has to be implemented that must return a snapshot of the current state in serializable form.

[source,go]
----
include::example$cloudstate/event.go[tag=snapshotter]
----
...
[source,go]
----
include::example$tck/cmd/tck_shoppingcart/shoppingcart.go[tag=snapshotter]
----
...
Snapshot handlers are declared by implementing the `cloudstate.SnapshotHandler` interface.

[source,go]
----
include::example$cloudstate/event.go[tag=snapshot-handler]
----
...
[source,go]
----
include::example$tck/cmd/tck_shoppingcart/shoppingcart.go[tag=handle-snapshot]
----

== Registering the entity

Once you've created your entity, you can register it with the `cloudstate.Cloudstate` server, by invoking the `Register` method of a Cloudstate instance.
